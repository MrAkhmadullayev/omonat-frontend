...
export default function ReceivableReportsPage() {
	...
	const {
		...
		user ? '/receivables/reports' : null,
		() => receivableApi.getReports(),
		{
			...
			datasets: [
				{
					...
					backgroundColor: ['#10b981', '#06b6d4'], // emerald va cyan
					hoverBackgroundColor: ['#059669', '#0891b2'],
					borderWidth: 0,
					cutout: '70%',
				},
			],
	...
	return (
		...
		>
			...
			<div className='grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'>
					...
					<Card className='relative overflow-hidden border-border/50 bg-background/80 backdrop-blur-sm h-full hover:shadow-md transition-shadow'>
						...
						<CardContent>
							...
							<div className='mt-2'>
								<Badge
									variant='secondary'
									className='bg-emerald-50 text-emerald-600 hover:bg-emerald-50 border-none font-semibold'
								>
									<CheckCircle2 className='w-3.5 h-3.5 mr-1.5' />{' '}
									{summary.totalAmount > 0
										? Math.round(
												(summary.totalReceived / summary.totalAmount) * 100,
											)
										: 0}
									% olingan
								</Badge>
				...
				<motion.div variants={itemVariants}>
					<Card className='relative overflow-hidden border-border/50 bg-background/80 backdrop-blur-sm h-full hover:shadow-md transition-shadow'>
						<div className='absolute top-0 right-0 w-20 h-20 bg-amber-500/5 rounded-bl-full -z-10'></div>
						<CardHeader className='pb-2'>
							<CardTitle className='text-sm font-bold text-muted-foreground'>
								Muddati o'tgan
							</CardTitle>
						...
						<CardContent>
							...
							<div className='mt-2'>
								<Badge
									variant='secondary'
									className='bg-amber-50 text-amber-600 hover:bg-amber-50 border-none font-semibold'
								>
									<AlertCircle className='w-3.5 h-3.5 mr-1.5' />{' '}
									{summary.overdueCount} ta haqqingiz
								</Badge>
							</div>
						</CardContent>
					</Card>
					...
					<Card className='relative overflow-hidden border-border/50 bg-background/80 backdrop-blur-sm h-full hover:shadow-md transition-shadow'>
						...
						<CardContent>
							<div className='flex items-baseline gap-1'>
								...
								<span className='text-sm font-bold text-cyan-600 mb-0.5'>
									kun
								</span>
							</div>
							<div className='mt-2'>
								<Badge
									variant='secondary'
									className='bg-cyan-50 text-cyan-600 hover:bg-cyan-50 border-none font-semibold'
								>
									<Calendar className='w-3.5 h-3.5 mr-1.5' /> Olish vaqti
								</Badge>
							</div>
						</CardContent>
					</Card>
						...
						<CardHeader className='flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/50 pb-4'>
							<div>
								<CardTitle className='text-base md:text-lg font-bold'>
									Berilgan sanalar bo'yicha
								</CardTitle>
								<CardDescription className='text-xs font-medium'>
									Barcha haqlaringiz dinamikasi
								</CardDescription>
							</div>
							<Tabs
								value={activePeriod}
				...
				<motion.div variants={itemVariants} className='lg:col-span-3'>
					<Card className='h-full border-border/50 shadow-sm flex flex-col'>
						<CardHeader className='border-b border-border/50 pb-4'>
							...
							<CardDescription className='text-xs font-medium'>
								Naqd va Karta orqali qabul qilinganlar ulushi
							</CardDescription>
						</CardHeader>
			...
			<motion.div variants={itemVariants}>
				<Card className='border-border/50 shadow-sm overflow-hidden mb-6'>
					<CardHeader className='bg-emerald-500/5 border-b border-emerald-500/10 py-4'>
						<CardTitle className='text-base md:text-lg font-bold flex items-center gap-2 text-emerald-600'>
					...
					<CardContent className='p-4 md:p-6'>
						<div className='grid gap-4 md:gap-6 md:grid-cols-3'>
							<div className='p-5 rounded-2xl bg-muted/30 border border-border/50 hover:border-emerald-500/20 transition-colors'>
								...
								<div className='space-y-4'>
									...
									<p className='text-xs text-muted-foreground leading-relaxed font-medium'>
										Sizning umumiy haqlaringizdan qariyb{' '}
										<strong>
											{summary.activeCount + summary.paidCount > 0
												? Math.round(
														(summary.paidCount /
										...
										qismi muvaffaqiyatli qabul qilingan.
									</p>
								</div>
							</div>

							<div className='p-5 rounded-2xl bg-muted/30 border border-border/50 hover:border-cyan-500/20 transition-colors'>
								<h4 className='text-sm font-bold text-foreground mb-4 flex items-center gap-2'>
								...
								<div className='space-y-2'>
									...
									<p className='text-xs text-muted-foreground leading-relaxed font-medium'>
										Berilgan pullarni qabul qilib olish uchun o'rtacha{' '}
							...
							<div className='p-5 rounded-2xl bg-amber-50/50 border border-amber-100/50 hover:border-amber-200 transition-colors'>
								...
								<div className='space-y-2'>
									<div className='flex items-end gap-1.5 mb-2'>
										...
										<span className='text-sm font-bold mb-1 text-amber-600/80'>
											ta muammoli haq
										</span>
									</div>
									<p className='text-xs text-amber-700/80 leading-relaxed font-medium'>
										...
										bo'lgan haqlaringiz muddati o'tgan. Qarzdorlar bilan
										bog'laning.
									</p>
								</div>
							</div>
